...
const WORKSPACE_ROOT_PACKAGE = '@mutantcat/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@mutantcat/')) throw new Error(`${normalized} must name an @mutantcat package`)
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: '@mutantcat/dsh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@mutantcat/', '')}-v`
  }
