...
该值必须是正的安全整数。它既提供模型自行报告阻塞的硬下限，也决定模型指引中指明的数值。生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-goal)是每个受支持字段的穷尽式真源。
...
- [生成的工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-goal)——模型接收的精确 schema。
...
生成的 [`get_goal`、`create_goal` 和 `update_goal` schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-goal)。成功结果是紧凑 JSON。变更会追加 goal 领域的持久 `goal/change` 事件，而不会将模型上下文加入队列。结果中的 `activation` 是实时观察值，绝不会成为回放权限依据。
