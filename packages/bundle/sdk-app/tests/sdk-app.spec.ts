...
describe('dsh-sdk-app bundle', () => {
  it('declares startup-gated JSON-RPC serving without overriding base HMR policy', () => {
    ...
    expect(manifest.dependencies).toHaveProperty('@mutantcat/dsh-sdk-jsonrpc-server')
    ...
    expect(rows.find(row => row.id === 'sdk-app-startup')?.name).toBe('@mutantcat/dsh-sdk-app')
