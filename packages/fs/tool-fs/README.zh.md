...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-fs)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-fs)——本包注册的穷尽式 schema。
...
模型会看到已生成的 [`read`、`read_image`、`write` 和 `edit` schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-fs)，参数使用 snake_case。图片工具只在持久附件存储已挂载时出现；schema 本身与路由无关，严格门禁在执行时拒绝。作用域工具限制可以为某个 agent 移除任一定义。
