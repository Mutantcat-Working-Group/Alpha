...
import * as Connection from '@mutantcat/dsh-client-connection'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "    host: '127.0.0.1'",
    '    port: 0',
    "- name: '@mutantcat/dsh-client-connection'",
    '- id: frontend',
    "  name: '@mutantcat/dsh-host-frontend-static'",
    '  config:',
    `    distIndex: '${distIndex}'`,
    '',
  ].join('\n'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@mutantcat/dsh-client-connection', Connection],
    ['@mutantcat/dsh-host-frontend-static', FrontendStatic],
  ])
