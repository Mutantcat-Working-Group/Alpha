...
import type { ClientTransportHooks } from '@mutantcat/dsh-client-connection/client'
...
const TYPERT_ONLY = webApp.pick(['@mutantcat/dsh-typert-registry'])
...
describe('TestClient (node environment)', () => {
  ...
  it('rethrows a row that fails to apply and lets the next client boot', async () => {
    ...
    await expect(TestClient.start({ roster: TYPERT_ONLY, provide: { '@mutantcat/dsh-typert-registry': failing } }, RemoteMock.create()))
  ...
  it('refuses a provide entry for the api-remotes row, whose services are the proxies', async () => {
    const roster = webApp.closure(['@mutantcat/dsh-api-remotes'])
    await expect(TestClient.start({ roster, provide: { '@mutantcat/dsh-api-remotes': { apply() {} } } }, RemoteMock.create()))
      .rejects.toThrow('@mutantcat/dsh-api-remotes cannot be provided; its remote.<ns> services are the tier\'s proxies')
  })
