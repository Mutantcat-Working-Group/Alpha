...
- [生成工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-subagent-control)——三个工具的 schema。
...
已生成的 [schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-subagent-control)：`send_message` 接受 `agent_id` 与 `message`；`interrupt_agent` 接受 `agent_id`；`list_agents` 接受可选的 `scope` 枚举。
