 ...
 * process-local registry lives in `@mutantcat/dsh-jobs-local`.
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@mutantcat/dsh-jobs is the abstract job registry seam; load an implementation such as @mutantcat/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
