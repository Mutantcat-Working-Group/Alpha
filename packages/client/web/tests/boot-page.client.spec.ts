...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@mutantcat/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@mutantcat/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@mutantcat/dsh-client-ui-layout')
    expect(el.textContent).toContain('@mutantcat/dsh-client-ui-tool')
