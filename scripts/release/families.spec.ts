...
describe('release families', () => {
  it('publishes all current experimental packages', () => {
    ...
    expect(members
      ...
      '@mutantcat/dsh-experimental-browser-use-runtime',
      '@mutantcat/dsh-experimental-browser-use-stagehand-native',
      '@mutantcat/dsh-experimental-client-ui-agent-team',
      '@mutantcat/dsh-experimental-computer-use-cua-driver-mcp',
      '@mutantcat/dsh-experimental-computer-use-cua-driver-native',
      ...
      '@mutantcat/dsh-experimental-webworker-packer',
      '@mutantcat/dsh-experimental-webworker-runtime',
    ])
  })
  ...
  it('excludes private applications from the publish set', () => {
    ...
    roots.push(root)
    write(join(root, 'apps/public/package.json'), '{"name":"@mutantcat/dsh-public","version":"0.0.1"}\n')
    write(join(root, 'apps/private/package.json'), '{"name":"@mutantcat/dsh-private","version":"0.0.1","private":true}\n')

    expect(releaseFamily('dsh').members(root).map(entry => entry.name)).toEqual(['@mutantcat/dsh-public'])
  })
  ...
  it('publishes unlisted experimental packages while retaining private exclusions', () => {
    ...
    write(join(root, 'packages/experimental/inspector/package.json'), JSON.stringify({
      name: '@mutantcat/dsh-experimental-inspector',
      version: '0.0.1',
      private: true,
    }))

    expect(releaseFamily('dsh').members(root).map(entry => entry.name)).toEqual([
      '@mutantcat/dsh-experimental-prototype',
    ])
  })
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@mutantcat/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@mutantcat/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@mutantcat/dsh-consumer', { dependencies: { '@mutantcat/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@mutantcat/dsh-library'),
      member('packages/a/zebra', '@mutantcat/dsh-zebra'),
    ]

    expect(dsh.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@mutantcat/dsh-library',
      '@mutantcat/dsh-consumer',
      '@mutantcat/dsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@mutantcat/dsh-left', { dependencies: { '@mutantcat/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@mutantcat/dsh-right', { dependencies: { '@mutantcat/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@mutantcat/dsh-consumer', { peerDependencies: { '@mutantcat/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@mutantcat/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@mutantcat/dsh-left', { peerDependencies: { '@mutantcat/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@mutantcat/dsh-right', { peerDependencies: { '@mutantcat/dsh-left': 'workspace:^' } }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@mutantcat/dsh-right', peer: '@mutantcat/dsh-left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@mutantcat/dsh-base', { peerDependencies: { '@mutantcat/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@mutantcat/dsh-consumer', {
        dependencies: { '@mutantcat/dsh-base': 'workspace:^' },
        peerDependencies: { '@mutantcat/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@mutantcat/dsh-base', peer: '@mutantcat/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@mutantcat/dsh-alpha', { peerDependencies: { '@mutantcat/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@mutantcat/dsh-bravo', { peerDependencies: { '@mutantcat/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@mutantcat/dsh-charlie', { dependencies: { '@mutantcat/dsh-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @mutantcat\/dsh-charlie -> @mutantcat\/dsh-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@mutantcat/dsh-alpha', { devDependencies: { '@mutantcat/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@mutantcat/dsh-zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@mutantcat/dsh-library')
    const vendored = member('vendor/cordis', '@mutantcat/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@mutantcat/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@mutantcat/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@mutantcat/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@mutantcat/dsh-library', { files: ['lib/index.js'] }),
