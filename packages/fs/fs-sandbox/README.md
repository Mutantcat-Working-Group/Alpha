...
- name: '@mutantcat/dsh-fs-sandbox'
  config:
    cwd: /absolute/path/to/workspace
- name: '@mutantcat/dsh-tool-fs'
```

The backend's config is unchanged from the local backend's (`cwd` resolution default and `diffBasisMaxBytes` overwrite bound); the [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-fs-sandbox) is the exhaustive source.
