...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @mutantcat/dsh-consumer references missing in-repo peer @mutantcat/dsh-missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@mutantcat/dsh-provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@mutantcat/dsh-consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
