...
import { RemoteMock, ok, openStream } from '@mutantcat/dsh-remote-mock'
...
const MODULES = '@mutantcat/dsh-client-modules'
const SIDEBAR = '@mutantcat/dsh-client-ui-sidebar'
const PARALLEL_PROBE = '@mutantcat/dsh-client-test-parallel-probe'
...
describe('TestClient (jsdom)', () => {
  ...
  it('boots separate client instances against their own mocks and keeps shared shims until the last dispose', async () => {
    ...
    await a.reload('@mutantcat/dsh-client-connection')
  ...
  it('creates no mount element when the roster cannot be loaded', async () => {
    const roster = ClientRoster.of([{ name: '@mutantcat/dsh-client-test-runtime-missing', inject: [], immediately: true }])
    const before = document.body.childElementCount
  ...
  it('reports the log when the connection never becomes ready', async () => {
    ...
    const roster = webApp.closure(['@mutantcat/dsh-api-workspace-controller'])
