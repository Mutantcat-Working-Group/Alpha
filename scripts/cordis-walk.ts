...
const MERGE_HEAD = /declare module ['"](?:@mutantcat\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@mutantcat/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@mutantcat/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@mutantcat/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@mutantcat/cordis'` Context merge
...
/**
 * Every event name a `declare module '@mutantcat/cordis'` Events merge declares in one
