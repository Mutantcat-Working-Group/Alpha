...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@mutantcat/dsh-client-' + short,
...
describe('source package uses', () => {
  ...
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@mutantcat/dsh-a/subpath'",
      "declare module '@mutantcat/dsh-client-ui-slots' {}",
      "const load = () => import('@mutantcat/dsh-b/remote')",
    ...
    expect([...uses].sort()).toEqual([
      '@mutantcat/dsh-a',
      '@mutantcat/dsh-b',
      '@mutantcat/dsh-client-ui-slots',
      'react',
    ])
    expect([...collectRuntimeSourcePackageUses('feature.tsx', [
      "import type { A } from '@mutantcat/dsh-a/subpath'",
      "declare module '@mutantcat/dsh-client-ui-slots' {}",
      "const load = () => import('@mutantcat/dsh-b')",
      'export const view = <div />',
    ].join('\n'))].sort()).toEqual([
      '@mutantcat/dsh-b',
      'react',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@mutantcat/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@mutantcat/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@mutantcat/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@mutantcat/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@mutantcat/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@mutantcat/dsh-api-stale', {
      external: ['@mutantcat/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@mutantcat/dsh-api-live', {
      external: ['@mutantcat/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@mutantcat/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@mutantcat/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@mutantcat/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@mutantcat/dsh-api-session-controller', {
      external: ['@mutantcat/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@mutantcat/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@mutantcat/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@mutantcat/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@mutantcat/dsh-client-ui', '@mutantcat/dsh-missing', '@mutantcat/dsh-missing'],
      inject: ['', '@mutantcat/dsh-a', '@mutantcat/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@mutantcat/dsh-api-a', {
      external: ['@mutantcat/dsh-api-b'],
      inject: ['@mutantcat/dsh-api-b'],
      runtimeSourceUses: { '@mutantcat/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@mutantcat/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@mutantcat/dsh-api-b', {
      external: ['@mutantcat/dsh-api-a'],
      inject: ['@mutantcat/dsh-api-a'],
      runtimeSourceUses: { '@mutantcat/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@mutantcat/dsh-api-a': ['packages/client/b/src/client.ts'] },
...
describe('manifest declarations', () => {
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@mutantcat/dsh-client-feature', '@mutantcat/dsh-missing'],
      inject: ['', '@mutantcat/dsh-agent', '@mutantcat/dsh-agent'],
      sourceUses: {
        '@mutantcat/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@mutantcat/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@mutantcat/dsh-agent': 'workspace:*',
      },
      peerDependencies: {
        '@mutantcat/dsh-client-ui-slots': 'workspace:^',
        '@mutantcat/cordis-plugin-loader': 'workspace:^',
      },
      devDependencies: {},
    })
    const slots = declaration('ui-slots', { dynamic: false })
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@mutantcat/dsh-missing'],
      inject: ['@mutantcat/dsh-agent'],
    })
