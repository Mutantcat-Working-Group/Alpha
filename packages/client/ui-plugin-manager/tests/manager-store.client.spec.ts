...
import type { BundleInfo, ChangeResult, ManagementError, PluginEntryId, PluginInfo, PluginInstallRequestId } from '@mutantcat/dsh-api-remotes/client'
...
const PLUGINS: PluginInfo[] = [
  ...
  { entryId: 'include:core' as PluginEntryId, moduleName: '@mutantcat/dsh-base', enabled: true, fiberPhase: 'active', readOnlyReason: 'management-required' },
]
...
describe('packageView', () => {
  it('joins a bundle with the entries its rows run as', () => {
    ...
    const protectedBundle: BundleInfo = {
      name: '@mutantcat/dsh-base', enabled: true, installed: false, optional: false, removable: false, readOnlyReason: 'management-required',
      error: { code: 'operation-error', diagnostic: 'broken' },
      rows: [{ rowId: 'core', moduleName: '@mutantcat/dsh-base', entryId: 'include:core' as PluginEntryId }, { rowId: 'gone', moduleName: 'x', entryId: 'include:gone' as PluginEntryId }],
      overrides: [],
    }
    expect(packageView(protectedBundle, PLUGINS)).toEqual({
      name: '@mutantcat/dsh-base', installed: false, optional: false, enabled: true, readOnlyReason: 'management-required',
      error: { code: 'operation-error', diagnostic: 'broken' },
      rows: [
        { rowId: 'core', moduleName: '@mutantcat/dsh-base', entryId: 'include:core', enabled: true, phase: 'active', readOnlyReason: 'management-required' },
