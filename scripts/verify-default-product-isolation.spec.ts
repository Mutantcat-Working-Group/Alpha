...
const roots: string[] = []
const experimental = '@mutantcat/dsh-experimental-prototype'
const core = '@mutantcat/dsh-core'
const base = '@mutantcat/dsh-base'
...
function fixture(): string {
  ...
  write(root, 'python/sdk-runtime/package.json', { name: '@mutantcat/dsh-python-runtime' })
...
describe('default product isolation', () => {
  ...
  it('ships an optional bundle switched off: its graph is outside the product, its name stays out of imports and defaults', () => {
    const root = fixture()
    const layer = '@mutantcat/dsh-experimental-layer'
  ...
  it('rejects experimental names absent from the inventory and paths with another package name', () => {
    const root = fixture()
    manifest(root, 'apps/cli/package.json', { dependencies: { '@mutantcat/dsh-experimental-missing': '*' } })
    ...
    const failures = verifyDefaultProductIsolation(root).failures.join('\n')
    expect(failures).toContain('@mutantcat/dsh-experimental-missing')
  ...
  it.each([
    ...
    [{ name: '@mutantcat/cordis-plugin-group', config: [{ name: experimental }] }],
    [{ name: '@mutantcat/cordis-plugin-include', config: { patches: [{ insert: [{ name: experimental }] }] } }],
  ...
  it('follows Include files while ignoring ordinary plugin config data', () => {
    ...
    expect(verifyDefaultProductIsolation(root).failures).toEqual([])
    write(root, patch, [{ name: '@mutantcat/cordis-plugin-include', config: { path: './nested.yml' } }])
  ...
  it('checks group contents after an id-only patch changes the composed Web tree', () => {
    const root = fixture()
    const web = '@mutantcat/dsh-web-app'
