...
import type { CommandResult } from '@mutantcat/dsh-commands/types'
import { CommandDefinitionId } from '@mutantcat/dsh-commands/brand'
import { createScope, scopeOf } from '@mutantcat/dsh-api-session-controller/client'
...
import { IconGoalOutline16 } from '@mutantcat/dsh-client-ui-primitives'
import type { ClientSessionContext, ConsumeTokenRequest, InputTriggerPick, InputTriggerSource, SubmitAttachment } from '@mutantcat/dsh-client-ui-input-trigger/client'
...
describe('candidates', () => {
  ...
  it('localizes canonical built-in and contribution descriptions on every candidate request', async () => {
    let locale = 'zh'
    const commands: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@mutantcat/dsh-command-compact'), name: 'compact', description: 'Compact older conversation history' },
  ...
  describe('menu presentation (design doc for #3567)', () => {
    ...
    const SHIPPED: CommandDescriptor[] = [
      { definitionId: CommandDefinitionId('@mutantcat/dsh-command-compact'), name: 'compact', description: 'Compact older conversation history' },
      { definitionId: CommandDefinitionId('@mutantcat/dsh-session-log-export'), name: 'export', description: 'Download this Session log as a ZIP archive' },
      { definitionId: CommandDefinitionId('@mutantcat/dsh-command-feedback'), name: 'feedback', description: 'Record feedback about this session', input: { hint: '<text>' } },
      { definitionId: CommandDefinitionId('@mutantcat/dsh-command-goal'), name: 'goal', description: 'Set or view the goal for a long-running task', input: { hint: '<objective>', attachments: true } },
      { definitionId: CommandDefinitionId('@mutantcat/dsh-permission-presets'), name: 'permission', description: 'Switch the permission preset (sandbox mode + approval policy)', input: { hint: '<preset>' } },
      { definitionId: CommandDefinitionId('@mutantcat/dsh-plan-mode'), name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
