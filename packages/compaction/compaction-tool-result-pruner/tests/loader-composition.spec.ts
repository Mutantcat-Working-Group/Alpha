  ...
  it('loads and resolves the flat YAML plugin shape', async () => {
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@mutantcat/dsh-session-projection') return SessionProjectionRegistry
        if (specifier === '@mutantcat/dsh-token-meter') return TokenMeter
        if (specifier === '@mutantcat/dsh-compaction-tool-result-pruner') return ToolResultPruner
