...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    expect(screen.queryByText('Alpha Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 24 24"]')).not.toBeNull()
    ...
    screen.getByText('Alpha Local Build')
  ...
  for (const plugin of ['@mutantcat/dsh-client-ui-layout', '@mutantcat/dsh-client-ui-sidebar', '@mutantcat/dsh-client-ui-conversation', '@mutantcat/dsh-client-ui-tool']) {
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp({ exclude: ['@mutantcat/dsh-client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__DSH_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@mutantcat/dsh-client-ui-chat')).toBe(false)
