 ...
 *
 * @module @mutantcat/dsh-api-settings-controller/src/credentials.ts
 */

import { Context } from '@mutantcat/cordis'
import { credentialRef } from '@mutantcat/dsh-credentials'
import type { CredentialProvider } from '@mutantcat/dsh-credentials'
import type { CredentialInfo } from '@mutantcat/dsh-credentials/types'
  ...
  private provider(): CredentialProvider {
    ...
    if (credentials === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'credentials service is absent: this deployment does not mount a credential provider (e.g. @mutantcat/dsh-credentials-local) in its composition',
        {},
      )
    }
    return credentials
