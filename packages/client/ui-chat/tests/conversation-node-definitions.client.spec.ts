...
import { LlmAttemptId } from '@mutantcat/dsh-llm/brand'
...
import { inspectRequestPrompt } from '@mutantcat/dsh-client-ui-conversation/client'
  ...
  it('shows the system node text as the request prompt card before the request messages', () => {
    const value = assembler([
      ...
      at(5, 'user/message', {
        ...
        source: { kind: 'plugin', plugin: '@mutantcat/dsh-system-prompt', form: 'snapshot' },
      }, { surfaceOp: 'append' }),
      at(6, 'request/header', {
        reason: 'initial',
        header: { config: { provider: 'fake', model: 'fake' } },
