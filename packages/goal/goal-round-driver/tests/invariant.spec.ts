...
import * as GoalSessionInvariant from '@mutantcat/dsh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@mutantcat/dsh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@mutantcat/dsh-invariants'
import SessionStore, { SessionId, type Session } from '@mutantcat/dsh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@mutantcat/dsh-goal-round-driver',
    }))
  })
