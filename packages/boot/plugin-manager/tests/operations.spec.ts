...
import { initProfile, readProfileManifest } from '@mutantcat/dsh-app-boot'
...
it('initializes missing profiles under the same lock and reports initialization', async () => {
  ...
  for (const profile of ['custom', 'web']) {
    ...
    expect(readProfileManifest('test', join(home, 'profiles', profile)).dsh?.profile?.bundles).toContain('@mutantcat/dsh-base')
  }
