...
export function collectSessionFormatMigrations(
  ...
  for (const discovered of globSync('packages/session/session-format-v*-to-v*/package.json', { cwd: scanRoot }).sort()) {
    ...
    const expectedPackageName = `@mutantcat/dsh-session-format-v${from}-to-v${to}`
  ...
  if (catalog.dependencies?.['@mutantcat/dsh-session'] !== undefined
    || catalog.peerDependencies?.['@mutantcat/dsh-session'] === undefined
    || catalog.devDependencies?.['@mutantcat/dsh-session'] === undefined) {
    throw new Error(
      'gen-session-format-catalog: catalog must share @mutantcat/dsh-session through peer + dev dependencies',
    )
  }
...
export function renderSessionFormatCatalog(
  ...
  return [
    ...
    ' */',
    '',
    "import { KNOWN_SESSION_EVENT_TYPES } from '@mutantcat/dsh-session'",
    "import { createSessionFormatCatalog } from '@mutantcat/dsh-session-format'",
