...
- name: '@mutantcat/dsh-lsp'
- name: '@mutantcat/dsh-lsp-stdio'
  config:
    servers:
      typescript:
...
`servers` must contain at least one entry with non-empty ids; timer budgets must be positive integers within Node's timer range, and byte caps must be positive. The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-lsp-stdio) is the exhaustive source for every accepted field.
