...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@mutantcat/dsh-subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@mutantcat/dsh-subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@mutantcat/dsh-subprocess-local')
    expect(bootstrap).toContain("import('@mutantcat/dsh/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@mutantcat/dsh-subprocess-local/runner')")
