 ...
 * @module @mutantcat/dsh-experimental-webworker-packer/src/pack
 */
...
} from '@mutantcat/dsh-experimental-webworker-runtime'
...
export { DEFAULT_ROOT } from '@mutantcat/dsh-experimental-webworker-runtime'
...
function sweepImage(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    try {
      ...
      const external = importer.startsWith('node_modules/') && !importer.startsWith('node_modules/@mutantcat/')
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@mutantcat/cordis is a peerDependency of every harness package),
