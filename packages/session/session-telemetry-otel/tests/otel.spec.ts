...
import { createAssistantMessage } from '@mutantcat/dsh-llm'
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionLogOffset, SessionSeq } from '@mutantcat/dsh-session'
import MessageFeedbackService from '@mutantcat/dsh-message-feedback'
import JsonlPersistence from '@mutantcat/dsh-session-persistence-jsonl'
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships only the submitted prefix through the real SDK pipeline', async () => {
    ...
    const ledger = records.filter(r => r.scope === '@mutantcat/dsh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@mutantcat/dsh-session-telemetry-otel/ops')
