...
import { ToolCallId, ReasoningEffortId } from '@mutantcat/dsh-llm'
...
import { assembleContextFor, type Agent } from '@mutantcat/dsh-agent'
...
import type { SubagentStartRequest } from '@mutantcat/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @mutantcat/dsh-jobs')
  })
