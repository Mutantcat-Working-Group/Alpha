...
import { SESSION_FORMAT_VERSION, SessionId, SessionSeq } from '@mutantcat/dsh-session'
...
vi.mock('@mutantcat/node-addon-system/flock', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@mutantcat/node-addon-system/flock')>()
  return {
