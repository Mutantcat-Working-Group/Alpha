...
```yaml
- name: '@mutantcat/dsh-browser-use'
- name: '@mutantcat/dsh-experimental-browser-use-playwright-mcp'
  config:
    mode: launch
    headless: true
```
...
The [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-experimental-browser-use-playwright-mcp) lists accepted fields. The profile or preset selects the browser mode. The child process clears inherited `PLAYWRIGHT_MCP_*` options so they cannot replace that configuration.
