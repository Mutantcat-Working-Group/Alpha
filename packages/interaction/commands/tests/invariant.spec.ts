...
import * as CommandInvariant from '@mutantcat/dsh-commands/invariant'
import InvariantRegistry, { InvariantError } from '@mutantcat/dsh-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@mutantcat/dsh-session'
import { CommandId } from '@mutantcat/dsh-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@mutantcat/dsh-commands',
    }))
  })
