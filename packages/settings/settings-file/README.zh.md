...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-settings-file)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
| — | 不发布运行时不变式伴生入口；文件往返、watcher 时序与原子写入行为由包测试证明，进程内提交关系归 `@mutantcat/dsh-settings` 所有。 |
...
- [生成配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-settings-file)——每个受支持配置字段及其源声明。
