...
function facts(manifest: PackageDependencyManifest): PackageDependencyFacts {
  return {
    ...
    workspaceNames: new Set([
      CORDIS,
      '@mutantcat/dsh-runtime',
      '@mutantcat/dsh-types',
      '@mutantcat/dsh-stale',
      '@mutantcat/schemastery',
    ]),
    allSourceUses: new Map([
      ['@mutantcat/dsh-runtime', ['packages/core/probe/src/index.ts']],
      ['@mutantcat/dsh-types', ['packages/core/probe/src/types.ts']],
    ]),
    hostRuntimeSourceUses: new Map([
      ['@mutantcat/dsh-runtime', ['packages/core/probe/src/index.ts']],
    ]),
    hostRuntimeExportUses: [{
      packageName: '@mutantcat/dsh-runtime',
      specifier: '@mutantcat/dsh-runtime',
      exportName: 'runtimeValue',
      sourcePath: 'packages/core/probe/src/index.ts',
      line: 1,
      column: 10,
      sourceLine: "import { runtimeValue } from '@mutantcat/dsh-runtime'",
    }],
...
describe('package dependency scope', () => {
  it('keeps the measured Host relay roster explicit', () => {
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@mutantcat/dsh-deque']).toEqual(['Deque'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@mutantcat/schemastery']).toEqual(['default'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@mutantcat/dsh-session/types']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@mutantcat/dsh-typert-protocol']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@mutantcat/dsh-scope']).toEqual([
      'carrierKeyOf', 'scopeOf', 'scopeTarget',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@mutantcat/dsh-typert-protocol']).toBeUndefined()
  })
...
describe('face-aware source classification', () => {
  ...
  it('identifies exact runtime exports without treating type imports as values', () => {
    const source = [
      ...
      "import { createLazyRequire as lazy } from '@mutantcat/dsh-lazy-require'",
      "import * as lazyModule from '@mutantcat/dsh-lazy-require'",
    ...
    expect(uses.map(({ specifier, exportName }) => ({ specifier, exportName }))).toEqual([
      { specifier: '@mutantcat/dsh-lazy-require', exportName: '*' },
      { specifier: '@mutantcat/dsh-lazy-require', exportName: 'createLazyRequire' },
...
describe('dependency sections', () => {
  ...
  it('validates every third-party range before writing any manifest in a repair batch', () => {
    ...
    roots.push(root)
    const valid = { ...facts({ name: '@mutantcat/dsh-first' }), manifestPath: 'first.json' }
    const base = facts({ name: '@mutantcat/dsh-second' })
  ...
  it('moves browser-only third-party imports to development dependencies without changing their ranges', () => {
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-probe',
      dependencies: { '@mutantcat/dsh-runtime': 'workspace:^', external: '^1.2.3' },
      devDependencies: { [CORDIS]: 'workspace:^', '@mutantcat/dsh-types': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
  ...
  it('does not leak repository configuration into captured dependency facts', () => {
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-client-locale',
    ...
    const subject: PackageDependencyFacts = {
      ...base,
      workspaceNames: new Set([...base.workspaceNames, '@mutantcat/dsh-api-remotes']),
    }
  ...
  it('requires non-workspace Host runtime imports in dependencies', () => {
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-probe',
      dependencies: { '@mutantcat/dsh-runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@mutantcat/dsh-types': 'workspace:^', external: '^1.0.0' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
  ...
  it('accepts Host dependencies, development-only inputs, and shared Cordis', () => {
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-probe',
      dependencies: {
        '@mutantcat/dsh-runtime': 'workspace:^',
        '@mutantcat/schemastery': 'workspace:^',
        external: '^1.0.0',
      },
      devDependencies: {
        '@mutantcat/dsh-types': 'workspace:^',
        [CORDIS]: 'workspace:^',
      },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
  ...
  it('lists managed Host runtime dependencies for fix review', () => {
    const subject = facts({ name: '@mutantcat/dsh-probe' })
    expect(formatManagedRuntimeDependencies({
      ...
      '  @mutantcat/dsh-probe -> @mutantcat/dsh-runtime: @mutantcat/dsh-runtime#runtimeValue',
    ])
  })
  ...
  it('reports an unapproved Host runtime export without rewriting its dependency section', () => {
    ...
    expect(safetyViolations).toEqual([
      'packages/core/probe/src/index.ts:1:10: @mutantcat/dsh-runtime#runtimeValue is not classified as '
      + 'safe or peer-required — import { runtimeValue } from \'@mutantcat/dsh-runtime\'',
    ])
    expect(fixPackageDependencies('/unused', state)).toEqual([])
    expect(manifest.dependencies).toEqual({ '@mutantcat/dsh-runtime': 'workspace:^' })
  })
  ...
  it('keeps an edge as a peer when one imported export requires shared identity', () => {
    ...
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
      peerRequiredHostDependencies: new Set(['@mutantcat/dsh-runtime']),
    }
    expect(collectHostDependencyExportPolicyViolations(
      [subject],
      subject.workspaceNames,
      {
        safeHostDependencyExports: {},
        peerRequiredHostExports: {
          '@mutantcat/dsh-runtime': ['runtimeValue'],
        },
      },
    )).toEqual([])
  ...
  it('reports wrong sections, workspace ranges, and stale peer metadata', () => {
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-probe',
      dependencies: { '@mutantcat/dsh-types': 'workspace:*' },
      devDependencies: { [CORDIS]: 'workspace:^', '@mutantcat/dsh-runtime': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:*', '@mutantcat/dsh-runtime': 'workspace:^' },
      peerDependenciesMeta: { '@mutantcat/dsh-missing': { optional: true } },
    }
    const state = {
      facts: [facts(manifest)], packages: [], policyViolations: [], workspaceNames: facts(manifest).workspaceNames,
    }
    ...
    expect(violations).toEqual(expect.arrayContaining([
      expect.stringContaining('@mutantcat/dsh-runtime'),
      expect.stringContaining('@mutantcat/dsh-types'),
      expect.stringContaining(`${CORDIS} must be matching peerDependencies + devDependencies`),
      expect.stringContaining('dependencies.@mutantcat/dsh-types must use workspace:^'),
      expect.stringContaining('peerDependenciesMeta.@mutantcat/dsh-missing has no matching'),
    ]))
  })
  ...
  it('repairs owned relationships without changing unrelated dependencies', () => {
    ...
    const manifest: PackageDependencyManifest = {
      name: '@mutantcat/dsh-probe',
      dependencies: { '@mutantcat/schemastery': 'workspace:*', external: '^1.0.0' },
      devDependencies: { [CORDIS]: 'workspace:^', '@mutantcat/dsh-runtime': 'workspace:^' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@mutantcat/dsh-runtime': 'workspace:^',
        '@mutantcat/dsh-stale': 'workspace:^',
      },
      peerDependenciesMeta: { '@mutantcat/dsh-stale': { optional: true } },
    }
