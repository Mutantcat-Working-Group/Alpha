...
/**
 * Keyless built-artifact smoke: plain Node imports `@mutantcat/dsh-lsp` and
 * `@mutantcat/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@mutantcat/cordis')
      const { default: Lsp } = await import('@mutantcat/dsh-lsp')
      const LspLocal = await import('@mutantcat/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@mutantcat/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@mutantcat/dsh-subprocess-local')
