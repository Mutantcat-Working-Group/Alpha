...
import { composeEntries, loadProfileDirectory } from '@mutantcat/dsh-app-boot'
...
it('retains one shared resource consumer in the Desktop Web profile', () => {
  ...
  try {
    ...
    expect(rows.filter(row => row.name === '@mutantcat/dsh-mcp-resources')).toEqual([
      { id: 'mcp-resources', name: '@mutantcat/dsh-mcp-resources' },
    ])
    expect(rows.filter(row => row.name === '@mutantcat/dsh-mcp-client')).toEqual([])
    expect(rows.find(row => row.id === 'webserver')).toMatchObject({ name: '@mutantcat/dsh-host-webserver' })
