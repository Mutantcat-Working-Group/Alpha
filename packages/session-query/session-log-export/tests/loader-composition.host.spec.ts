...
import * as SessionLogDownload from '@mutantcat/dsh-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@mutantcat/dsh-session-log-export'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@mutantcat/dsh-session-log-export', SessionLogDownload],
    ])
    ...
    expect(context.commands.list(agent)).toContainEqual({
      definitionId: '@mutantcat/dsh-session-log-export',
