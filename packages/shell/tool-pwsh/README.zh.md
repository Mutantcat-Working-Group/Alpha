...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-pwsh)是每个受支持字段及其 JSDoc 的穷尽式真源；生成的[工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-pwsh)携带完整参数 schema。
...
没有 PowerShell 执行器的组合永远不会激活该工具，且注入的服务（`tools`、`shell`、`systemPrompt`、`shellEnv`）必须全部存在。没有任务运行时的后台调用会以 `background jobs unavailable: load @mutantcat/dsh-jobs and @mutantcat/dsh-tool-jobs` 失败；没有沙箱执行器时的 `sandbox_permissions` 会以 `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)` 失败。
...
- [生成的工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-pwsh)——`pwsh` 参数 schema 的确切内容。
- [生成的配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-pwsh)——每个受支持配置字段及其源声明。
...
模型会看到生成的 [`pwsh` schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-pwsh)。按 agent 作用域实施的工具限制可以移除该 agent 的定义。
...
- **没有持久 shell**——每次调用都启动全新的 `pwsh -Command`；持久 shell 对应物是 [`@mutantcat/dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.zh.md)，它跨调用保持一个按所有者隔离的 pwsh 存活。
