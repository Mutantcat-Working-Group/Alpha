 ...
 * Everything else under @mutantcat/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@mutantcat\/dsh-(?:file-reference|session|llm|tools|brand|deque|output-retention|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@mutantcat\/dsh-token-meter\/client$|@mutantcat\/dsh-host-open-in-app\/shared$|@mutantcat\/dsh-agent-presets\/display$|@mutantcat\/dsh-spill-policy\/notice$)/
...
const GENERATED_REMOTE = /^@mutantcat\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
function clientConfig(id: string, entry: string, clientBanner?: (fileName: string) => string | undefined): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @mutantcat value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@mutantcat/')) return null
...
function clientInputIsolation(id: string): {
  ...
  const experimental = id.startsWith('@mutantcat/dsh-experimental-')
