...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-sandbox-local) is the exhaustive source for every accepted field and its JSDoc.
...
The `@mutantcat/node-addon-system/landlock-run` API supplies the platform launcher, functional probe, and grant vocabulary; this provider maps mode to grants only, keeping path resolution and probe parsing with the versioned binary.
