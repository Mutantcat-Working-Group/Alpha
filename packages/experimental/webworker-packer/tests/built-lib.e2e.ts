...
const packageNames = new Set(packages.map(name => `@mutantcat/dsh-experimental-${name}`))
...
it('loads both tarballs through plain Node and mounts their base image and overlay', { retry: 0 }, async (test) => {
  ...
  for (const name of packages) {
    ...
    const directory = join(root, 'node_modules/@mutantcat', `dsh-experimental-${name}`)
  ...
  const script = `
    ...
    import { fileURLToPath } from 'node:url'
    import * as packer from '@mutantcat/dsh-experimental-webworker-packer'
    import * as runtime from '@mutantcat/dsh-experimental-webworker-runtime'
    import * as client from '@mutantcat/dsh-experimental-webworker-runtime/client'
    for (const name of ['webworker-packer', 'webworker-runtime']) {
      assert.equal(import.meta.resolve('@mutantcat/dsh-experimental-' + name),
        new URL('./node_modules/@mutantcat/dsh-experimental-' + name + '/lib/index.js', import.meta.url).href)
    }
    assert.equal(typeof client.connectWorkerHost, 'function')
    const worker = fileURLToPath(import.meta.resolve('@mutantcat/dsh-experimental-webworker-runtime/worker'))
