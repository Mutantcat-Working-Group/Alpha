 ...
 * @module @mutantcat/dsh-jobs-local
 */
...
import { deadline, timeoutOf } from '@mutantcat/dsh-timeout'
import { JobRegistry, JobId } from '@mutantcat/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@mutantcat/dsh-jobs'
 ...
 * `@mutantcat/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @mutantcat/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @mutantcat/dsh-agent)')
    }
