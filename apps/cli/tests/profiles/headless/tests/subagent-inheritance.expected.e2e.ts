...
import { createMessage, createUserMessage, ReasoningEffortId } from '@mutantcat/dsh-llm'
import { SessionSeq, SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@mutantcat/dsh-session'
...
async function seedReadOnlyParent(root: string, cwd: string): Promise<void> {
  ...
  const events: SessionEvent[] = [
    ...
    { type: 'step/start', seq: SessionSeq(1), time: 11, data: { turn: 1, step: 1 } },
    {
      ...
      data: { turn: 1, step: 1, message: createMessage({ role: 'system', content: [], source: { kind: 'plugin', plugin: '@mutantcat/dsh-system-prompt' } }) },
      surfaceOp: 'append',
    },
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@mutantcat/dsh-system-prompt') return []
