...
import type { Context, Plugin, RegistryService } from '@mutantcat/cordis'
import type { ClientModuleLoader, ClientModuleLoaderTarget } from '@mutantcat/dsh-client-modules/client'
...
function experimentalClientReferences(roster: Awaited<ReturnType<typeof readClientRoster>>): string[] {
  return [
    ...
    ...roster.modules,
  ].filter(name => name.startsWith('@mutantcat/dsh-experimental-'))
}
...
it('activates the actual default Client registry without experimental packages', async (test) => {
  await withDefaultWeb(test, async ({ url, request }) => {
    ...
    try {
        ...
        Object.defineProperty(globalThis, '__ModuleLoader__', {
          ...
          set(target: ClientModuleLoaderTarget) {
            ...
            target.create = function (options) {
              const cordis = options.staticModules['@mutantcat/cordis'] as { RegistryService: typeof RegistryService }
      ...
      expect(roster.plugins.some(plugin => plugin.modules.includes('@mutantcat/dsh-client-ui-layout'))).toBe(true)
