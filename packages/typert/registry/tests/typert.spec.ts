...
} from '@mutantcat/dsh-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@mutantcat/dsh-tools', 'ToolInput')).toBe('@mutantcat/dsh-tools#ToolInput')
    expect(typertPackageKey('@mutantcat/dsh-tools', 'host')).toBe('@mutantcat/dsh-tools#host')
    expect(ctx.typert.get('@mutantcat/dsh-tools#ToolInput')).toMatchObject({
      package: '@mutantcat/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@mutantcat/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.create())
    expect(ctx.typert.getPackage('@mutantcat/dsh-tools', 'host')).toMatchObject({
      key: '@mutantcat/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@mutantcat/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@mutantcat/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@mutantcat/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@mutantcat/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.create())
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@mutantcat/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(create).toHaveBeenCalledOnce()
    expect(ctx.typert.resolve('@mutantcat/dsh-tools#ToolInput').schema).toBe(schema)
    expect(create).toHaveBeenCalledOnce()
    expect(() => ctx.typert.resolve('@mutantcat/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@mutantcat/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@mutantcat/dsh-tools#ToolInput')).not.toBe(projected)
  })
