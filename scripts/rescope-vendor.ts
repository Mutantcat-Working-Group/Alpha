/**
 * Rescope the vendored Cordis packages into the `@mutantcat` scope, and undo
 ...
 * `@mutantcat/dsh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@mutantcat/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@mutantcat/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@mutantcat/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@mutantcat/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@mutantcat/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@mutantcat/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@mutantcat/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@mutantcat/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@mutantcat/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  ...
  { file: 'vendor/hmr/package.json', text: '"name": "@mutantcat/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@mutantcat\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@mutantcat/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@mutantcat/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@mutantcat/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@mutantcat/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@mutantcat/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@mutantcat/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@mutantcat` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@mutantcat\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@mutantcat/cordis']
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@mutantcat/\')) {',
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@mutantcat/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@mutantcat/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @mutantcat, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@mutantcat\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@mutantcat/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@mutantcat\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.sourceDirectory}](\${row.sourceDirectory}/) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@mutantcat/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
