...
import { toolPairingBalancedAfter, toolPairingBalancedBefore } from '@mutantcat/dsh-compaction'
import { createUserMessage, createSystemMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, resolveRetryPolicy , createMessage } from '@mutantcat/dsh-llm'
import type { ContentBlock, GenerateOptions, LlmResolvedModelInfo, ResolvedRetryPolicy, StreamChunk } from '@mutantcat/dsh-llm'
import { ToolCallId, LlmAdapter } from '@mutantcat/dsh-llm'
...
import * as AgentInvariant from '@mutantcat/dsh-agent/invariant'
import * as AgentLoopInvariant from '@mutantcat/dsh-agent-loop/invariant'
import { BasicCompactionEngine } from '@mutantcat/dsh-compaction-basic'
import TokenMeter from '@mutantcat/dsh-token-meter'
import * as LlmRetry from '@mutantcat/dsh-llm-retry'
import { Session, SessionId, type SessionEvent, type SurfaceEvent } from '@mutantcat/dsh-session'
    ...
    try {
      ...
      ctx.on('agent/request-error', async ({ agent, turn, step }, next) => {
        ...
        agent.session.append('system/message', {
          turn,
          step,
          message: createSystemMessage('retry guidance', '@mutantcat/dsh-system-prompt'),
