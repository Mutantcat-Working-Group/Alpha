...
import { SHIPPED_PRESET_ROOT } from '@mutantcat/dsh-agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@mutantcat/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@mutantcat/dsh-base', '@mutantcat/dsh-web-app'])
    ...
    for (const name of ['@mutantcat/dsh-pwsh-sandbox', '@mutantcat/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@mutantcat/dsh-base'])
