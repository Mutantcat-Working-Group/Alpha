...
import type { SettingsDescriptor } from '@mutantcat/dsh-settings'
import { RemoteError, remoteErrorOf, remoteMethods } from '@mutantcat/dsh-typert-protocol'
  ...
  it('reports the actionable configuration error while no settings provider is mounted', async () => {
    ...
    for (const call of calls) {
      ...
      expect(remoteErrorOf(failure)).toMatchObject({
        ...
        message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @mutantcat/dsh-settings-file) in its composition',
        details: {},
      })
    }
  })
