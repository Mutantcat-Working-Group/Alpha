...
- name: '@mutantcat/dsh-experimental-agent-team'
- name: '@mutantcat/dsh-experimental-tool-agent-team'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-experimental-agent-team) is the exhaustive source for every accepted field and its JSDoc.
...
Each peer delivery adds the sender prefix plus message content to the target history. Task and roster mutations add no model tokens; their model-facing representation belongs to `@mutantcat/dsh-experimental-tool-agent-team` results.
