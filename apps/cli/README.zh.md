...
`dsh.profile.bundles` 中列出的组合包先从 dsh 安装目录解析（`@mutantcat/dsh-base`、`@mutantcat/dsh-web-app`、`@mutantcat/dsh-headless`、`@mutantcat/dsh-sdk-app`、`@mutantcat/dsh-sdk-minimal`、`@mutantcat/dsh-acp-app`），再从 profile 自身的 `node_modules` 解析；pnpm 会将树外插件安装到该目录。
...
`@mutantcat/dsh/profile-boot` 导出向 Desktop Host 提供共享 profile 生命周期。已解析的应用 profile 为运行时包解析指定自己的安装锚点，同时沿用 Harness home patch、代理环境、遥测开关、patch 热重载和有界关闭。
