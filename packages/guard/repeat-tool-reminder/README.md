...
```yaml
- name: '@mutantcat/dsh-repeat-tool-reminder'
  config:
...
Invalid configuration fails at startup with a clear error — an empty `thresholds` list, a repeat count below 2, or a duplicate — never a silent change of behavior. The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-repeat-tool-reminder) documents every accepted value.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-repeat-tool-reminder) — every accepted config field and its source declaration.
