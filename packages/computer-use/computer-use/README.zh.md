...
服务没有配置项。提供方插件注入 `computerUse` 并调用 `ctx.computerUse.register(ComputerUseProviderName(name))`；该品牌类型从 `@mutantcat/dsh-computer-use/brand` 导出。返回的 effect 清理函数释放此次注册。
