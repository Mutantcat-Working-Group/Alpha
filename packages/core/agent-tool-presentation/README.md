...
```yaml
- name: '@mutantcat/dsh-agent-tool-presentation'
  config:
    mode: ptc
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-agent-tool-presentation) is the exhaustive source for every accepted field. `mode` is required rather than defaulted because a preset without this row inherits the deployment default.
