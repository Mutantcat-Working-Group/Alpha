...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@mutantcat/dsh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@mutantcat/dsh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@mutantcat/dsh-session-title', SessionTitleService],
    ['@mutantcat/dsh-session-title-first-prompt-llm', providerPlugin],
  ])
