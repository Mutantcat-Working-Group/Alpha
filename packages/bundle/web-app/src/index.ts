/**
 * @mutantcat/dsh-web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection, auditStartupEntries } from '@mutantcat/dsh-app-boot'
import type {} from '@mutantcat/dsh-client-connection'
import * as FrontendStatic from '@mutantcat/dsh-host-frontend-static'
import { launchedThroughSsh, launchEnvironmentOf } from '@mutantcat/dsh-launch-environment'
import { scrubbedParentEnv } from '@mutantcat/dsh-subprocess'
...
function resolveDistIndex(): string {
  ...
  try {
    return join(dirname(require.resolve('@mutantcat/dsh-web-frontend/package.json')), 'dist', 'index.html')
  } catch {
    /* v8 ignore next 2 -- reachable only when the frontend package is absent from the checkout */
    throw new Error('web-app: @mutantcat/dsh-web-frontend is not resolvable from this composition')
  }
}
