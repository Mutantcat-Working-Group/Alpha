...
import { OfficeSourceKey, type OfficeToPdfRequest } from '@mutantcat/dsh-office-to-pdf'
import * as LibreOfficeProvider from '@mutantcat/dsh-office-to-pdf'
...
it('loads one shared conversion row and retains caller-owned PDFs after disposal', async () => {
  ...
  const rows = loadOverlayPatches('conversion-test', fileURLToPath(new URL('../cordis.patch.yml', import.meta.url)))
    ...
    .filter(row => row.name === '@mutantcat/dsh-office-to-pdf')
  ...
  ctx.loader.internal = {
    ...
    async import(specifier: string) {
      if (specifier !== '@mutantcat/dsh-office-to-pdf') throw new Error(`Unexpected plugin: ${specifier}`)
