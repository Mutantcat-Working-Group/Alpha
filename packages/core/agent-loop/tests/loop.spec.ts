...
import LlmRuntime, { createUserMessage, ToolCallId, LlmError, ReasoningEffortId, StreamChunk, expandAssistantStream } from '@mutantcat/dsh-llm'
import type { GenerateOptions } from '@mutantcat/dsh-llm'
import SessionStore, { SessionId, TurnEndReason } from '@mutantcat/dsh-session'
import SystemPrompt from '@mutantcat/dsh-system-prompt'
import ToolRuntime, { defineContentToolFixture } from '@mutantcat/dsh-tools'
import AgentRegistry, { type Agent, type AssistantStreamFrame } from '@mutantcat/dsh-agent'
...
describe('agent loop', () => {
  ...
  it('materializes changed runtime context at the history tail without rewriting the system header', async () => {
    ...
    const contextEvents = () => agent.session.snapshotEvents().flatMap(event =>
      event.type === 'user/message'
        ...
        && event.data.source.plugin === '@mutantcat/dsh-system-prompt'
        ? [event]
        : [])
  ...
  it('re-emits unchanged runtime context when a surface replacement removed the retained snapshot', async () => {
    ...
    expect(adapter.requests[1]?.messages.some(message =>
      ...
      && message.source.plugin === '@mutantcat/dsh-system-prompt')).toBe(true)
  })
  ...
  it('clears compacted runtime context after the active set becomes empty', async () => {
    ...
    const clearing = adapter.requests[1]?.messages.find(message =>
      message.role === 'user'
      && message.source.kind === 'plugin'
      && message.source.plugin === '@mutantcat/dsh-system-prompt')
  ...
  it('does not clear runtime context after an unrelated replacement', async () => {
    ...
    expect(adapter.requests[0]?.messages.some(message =>
      message.role === 'user'
      && message.source.kind === 'plugin'
      && message.source.plugin === '@mutantcat/dsh-system-prompt')).toBe(false)
  })
  ...
  it('replaces a malformed retained runtime-context message with the current complete snapshot', async () => {
    ...
    agent.session.append('user/message', createUserMessage({
      ...
      source: { kind: 'plugin', plugin: '@mutantcat/dsh-system-prompt' },
    }), { surfaceOp: 'append' })
