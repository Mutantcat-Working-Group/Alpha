 ...
 * execution pipeline.
 * @module @mutantcat/dsh-tools
 */

import { Context, Service } from '@mutantcat/cordis'
import z from '@mutantcat/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@mutantcat/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@mutantcat/dsh-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@mutantcat/dsh-llm'
import { HarnessError } from '@mutantcat/dsh-llm'
import type { Agent } from '@mutantcat/dsh-agent'
import type { UserMessage } from '@mutantcat/dsh-session'
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@mutantcat/dsh-util-values'
import type { PromptSection, ToolProviderResult } from '@mutantcat/dsh-system-prompt'
import type { PtcRuntime } from '@mutantcat/dsh-ptc-runtime'
import type {} from '@mutantcat/dsh-sandbox-policy'
...
// lives in its own UI-facing module; re-export it so `@mutantcat/dsh-tools`
...
declare module '@mutantcat/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@mutantcat/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@mutantcat/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@mutantcat/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@mutantcat/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@mutantcat/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requirePtcRuntime(mode: ToolPresentationMode): PtcRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a PTC runtime — load a ctx.ptcRuntime implementation (e.g. @mutantcat/dsh-ptc-runtime-node) or set tools mode to "native"`)
    }
