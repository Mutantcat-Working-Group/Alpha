...
```yaml
- id: tool-goal
  name: '@mutantcat/dsh-tool-goal'
  config:
    blockedAfterConsecutiveRounds: 3
```

The value must be a positive safe integer. It supplies both the hard lower bound on model self-blocking and the number named in model guidance. The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-goal) is the exhaustive source for every accepted field.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-goal) — the exact schemas the model receives.
