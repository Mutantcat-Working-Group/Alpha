...
describe('tierExternalDeps', () => {
  it('limits the LibreOffice exception to its reviewed package identity and MPL terms', () => {
    ...
    for (const dependency of [
      ...
      { name: '@mutantcat/dsh-libreoffice-kit', license: 'MPL-2.0' },
  ...
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@mutantcat/dsh-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@mutantcat/dsh-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@mutantcat/dsh-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@mutantcat/dsh-cli', dependencies: { 'cli-lib': '^1', '@mutantcat/dsh-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@mutantcat/dsh-tui', dependencies: { shared: '^1', '@mutantcat/dsh-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@mutantcat/dsh-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@mutantcat/dsh-cli')).toBe(false)
  })
})
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@mutantcat/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
