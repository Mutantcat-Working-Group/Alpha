...
describe('next package benchmark graph', () => {
  it('applies a source-derived candidate without changing the filesystem', () => {
    ...
    const facts: PackageDependencyFacts = {
      ...
      manifest,
      workspaceNames: new Set(['@mutantcat/cordis', '@f/probe', '@f/runtime', '@f/types']),
  ...
  it('finds reachable unconfigured packages with non-Cordis peers', () => {
    const index = new Map([
      ['@mutantcat/dsh', new Map([['1.0.0', {
        name: '@mutantcat/dsh', version: '1.0.0', dependencies: { '@f/a': '^1.0.0', '@f/b': '^1.0.0' },
      }]])],
      ['@f/a', new Map([['1.0.0', {
        name: '@f/a', version: '1.0.0', peerDependencies: { '@f/runtime': '^1.0.0' },
      }]])],
      ['@f/b', new Map([['1.0.0', {
        name: '@f/b', version: '1.0.0', peerDependencies: { '@mutantcat/cordis': '^4.0.0' },
      }]])],
    ...
    expect(discoverBenchmarkCandidates(
      index,
      new Map([['@mutantcat/dsh', '1.0.0'], ['@f/a', '1.0.0'], ['@f/b', '1.0.0']]),
      release,
      new Set(),
    )).toEqual(['@f/a'])
  })
