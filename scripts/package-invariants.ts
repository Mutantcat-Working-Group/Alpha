...
function checkManifest(
  ...
  if (owner.packageName === '@mutantcat/dsh-invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@mutantcat/dsh-invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@mutantcat/dsh-invariants must not be a peerDependency under this package dependency policy'
      : '@mutantcat/dsh-invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@mutantcat/dsh-invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@mutantcat/dsh-invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@mutantcat/dsh-invariants'
