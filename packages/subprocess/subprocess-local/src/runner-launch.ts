...
import type { SubprocessSpawnSpec } from '@mutantcat/dsh-subprocess'
import { childEnv } from './spawn.ts'
import { controlEnvironment } from './control-spawn.ts'
import { SUBPROCESS_CONTROL_FD } from '@mutantcat/dsh-subprocess/control'
...
export function spawnRunnerInvocation(): RunnerInvocation {
  ...
  if (extname(fileURLToPath(import.meta.url)) !== '.ts') {
    return [process.execPath, fileURLToPath(import.meta.resolve('@mutantcat/dsh-subprocess-local/runner'))]
  }
  return [
    process.execPath,
    '--import',
