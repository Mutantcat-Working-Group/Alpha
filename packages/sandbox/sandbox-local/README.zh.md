...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-sandbox-local)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
`@mutantcat/node-addon-system/landlock-run` API 提供平台 launcher、功能探测与授权词汇；此提供方只做模式到授权的映射，把路径解析与探测解析保留在带版本的 binary 中。
