...
const SESSION_PACKAGE = '@mutantcat/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@mutantcat/dsh-session/types'
 ...
 * top-level declaration (in `@mutantcat/dsh-session`) and any declaration
 * merge inside a `declare module '@mutantcat/dsh-session/types'` block. Both forms
