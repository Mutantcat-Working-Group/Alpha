...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-fs-search)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-fs-search)——本包注册的穷尽式 schema。
...
glob 描述声明了配置的超过上限排序方式。生成的 [`glob` 和 `grep` schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-fs-search) 使用 `sampleOverCapGlobResults: true`；工具无条件注册。
