...
function fixture(edges: Array<[number, number]>): string {
  ...
  for (const [from, to] of edges) {
    ...
    const name = `@mutantcat/dsh-session-format-v${from}-to-v${to}`
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      dependencies: from === 0
        ? { '@mutantcat/dsh-session-format': 'workspace:^' }
        : {
          '@mutantcat/dsh-session-format': 'workspace:^',
          [`@mutantcat/dsh-session-format-v${from - 1}-to-v${from}`]: 'workspace:^',
        },
    }))
  }
  ...
  writeFileSync(join(catalog, 'package.json'), JSON.stringify({
    ...
    peerDependencies: { '@mutantcat/dsh-session': 'workspace:^' },
    devDependencies: { '@mutantcat/dsh-session': 'workspace:^' },
  }))
  return root
}
...
describe('session format catalog generator', () => {
  ...
  it('discovers one complete adjacent chain and renders direct imports', () => {
    ...
    expect(output).toContain("from '@mutantcat/dsh-session-format-v0-to-v1'")
    ...
    expect(output).toContain("from '@mutantcat/dsh-session'")
  ...
  it('requires every later edge to depend on the package that owns its source codec', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    delete manifest.value.dependencies['@mutantcat/dsh-session-format-v0-to-v1']
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must depend on @mutantcat\/dsh-session-format-v0-to-v1/)
  })

  it('requires the package name to identify its declared adjacent edge', () => {
    const root = fixture([[0, 1], [1, 2]])
    const manifest = edgeManifest(root, 1, 2)
    manifest.value['name'] = '@mutantcat/dsh-session-format-other'
    writeFileSync(manifest.path, JSON.stringify(manifest.value))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/name must be @mutantcat\/dsh-session-format-v1-to-v2/)
  })
  ...
  it('requires the catalog to share the installed Session package as a peer', () => {
    ...
    manifest.dependencies['@mutantcat/dsh-session'] = 'workspace:^'
    delete manifest.peerDependencies['@mutantcat/dsh-session']
    writeFileSync(path, JSON.stringify(manifest))

    expect(() => collectSessionFormatMigrations(root, 2))
      .toThrow(/must share @mutantcat\/dsh-session through peer \+ dev dependencies/)
  })
})
