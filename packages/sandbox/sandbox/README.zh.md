...
```yaml
- id: sandbox
  name: '@mutantcat/dsh-sandbox-local'     # the per-platform backend provider (ctx.sandbox)
- id: sandbox-policy
  name: '@mutantcat/dsh-sandbox-policy'    # the deployment default mode and workspace-write root
  config:
...
- id: bash
  name: '@mutantcat/dsh-bash-sandbox'      # the confined executor behind ctx.shell
```
