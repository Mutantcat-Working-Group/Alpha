...
import { RemoteMock, frames, ok, openStream } from '@mutantcat/dsh-remote-mock'
...
const API_ROSTER = webApp.closure(['@mutantcat/dsh-api-remotes'])
...
describe('remote proxies over a booted client', () => {
  it('boots the Office Remote subset from source and routes calls through its mock', async () => {
    const name = '@mutantcat/dsh-api-remotes'
  ...
  it('drops the api-remotes row and answers ctx.remote.<ns>.<method> with the mock value, logging positional args', async () => {
    ...
    expect([...client.ctx.loader.entries()].map(entry => entry.options.name)).not.toContain('@mutantcat/dsh-api-remotes')
