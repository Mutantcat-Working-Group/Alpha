...
import type { Session, SessionHeader, SessionId } from '@mutantcat/dsh-session'
import type { SessionProjectionRegistry } from '@mutantcat/dsh-session-projection'
import type { SessionProjectionCache } from '@mutantcat/dsh-session-projection-cache'
import type { SessionObservation, SessionQueryEngine } from '@mutantcat/dsh-session-query'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @mutantcat/dsh-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @mutantcat/dsh-session)',
  ...
  if (query === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionQuery service (load @mutantcat/dsh-session-query)',
