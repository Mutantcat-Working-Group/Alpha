...
生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-todo)是每个受支持字段的穷尽式真源。
...
- [生成的工具目录](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-todo)——模型接收的 `todo_write` schema。
- [生成的配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-tool-todo)——每个受支持配置字段及其源声明。
...
模型会看到生成的 [`todo_write` schema](../../../docs/tool-catalog.zh.md#mutantcatdsh-tool-todo)：一个对象，含一个必填的 `todos` 数组，元素为 `{ content, status }`，其中 `status` 为 `pending`、`in_progress` 或 `completed`。描述是组合后的整表指令，其活跃状态条款跟随 `allowParallelInProgress`。
