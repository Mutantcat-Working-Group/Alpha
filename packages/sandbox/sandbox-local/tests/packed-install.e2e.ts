...
const platformPackageName = `@mutantcat/node-addon-system-linux-${process.arch}`

const NATIVE_PACKAGE_PREFIX = '@mutantcat/node-addon-system'
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@mutantcat/node-addon-system/landlock-run'
