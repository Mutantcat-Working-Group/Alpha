...
const roots: string[] = []
const experimental = '@mutantcat/dsh-experimental-example'
...
describe('installed default-product isolation', () => {
  ...
  it('skips the entry package\'s optional bundles, which must be installed, and still rejects other experimental edges', () => {
    ...
    const missing = writePackage(root, '@mutantcat/dsh-missing', { dependencies: { absent: '1.0.0' } })
    expect(() => verifyInstalledProductIsolation(missing, ['absent'])).toThrow('optional bundle is missing: @mutantcat/dsh-missing -> absent')
  })
