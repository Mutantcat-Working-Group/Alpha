...
无效配置会在启动时以清晰错误失败——空的 `thresholds` 列表、小于 2 的重复次数或重复值——绝不会静默改变行为。生成的[配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-repeat-tool-reminder)记录每个受支持的值。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#mutantcatdsh-repeat-tool-reminder)——每个受支持配置字段及其源声明。
