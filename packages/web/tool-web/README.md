...
```yaml
- name: '@mutantcat/dsh-web'
- name: '@mutantcat/dsh-web-search-exa'
- name: '@mutantcat/dsh-tool-web'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-web) is the exhaustive source for every accepted field and its JSDoc. `searchMaxQueries` bounds the accepted array before exact-string deduplication and provider fan-out; validation rejects an oversized array before any search starts. The timeout budgets attach to each tool definition and are enforced by [`@mutantcat/dsh-tool-call-timeout-policy`](../../guard/timeout-policy/README.md); the model-facing schemas expose no timeout argument.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-web) — the exact `web_search` and `web_fetch` schemas.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-web) — every accepted config field and its source declaration.
...
The model sees the generated [`web_search` and `web_fetch` schemas](../../../docs/tool-catalog.md#mutantcatdsh-tool-web). Result-count and timeout budgets are deployment settings, not model arguments.
