 ...
 * not built (`pnpm --filter @mutantcat/dsh-client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@mutantcat/cordis'
import { stubSettingsScope } from '@mutantcat/dsh-client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@mutantcat/dsh-client-ui-conversation/client'
import { SlotRegistry } from '@mutantcat/dsh-client-ui-renderer/client'

const PLUGIN_ID = '@mutantcat/dsh-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@mutantcat/dsh-client-store', await import('@mutantcat/dsh-client-store')],
      ['@mutantcat/dsh-client-ui-conversation/client', await import('@mutantcat/dsh-client-ui-conversation/client')],
      ['@mutantcat/dsh-client-ui-primitives', await import('@mutantcat/dsh-client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@mutantcat/dsh-client-locale/client')
