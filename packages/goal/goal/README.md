...
`defaultMaxGoalRounds` must be a positive safe integer; a create request that names its own cap overrides it. The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-goal) is the exhaustive source for every accepted field.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-goal) — every accepted config field and its source declaration.
