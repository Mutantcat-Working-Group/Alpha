...
import ToolRuntime, { RUN_CODE_NAME, defineContentToolFixture } from '@mutantcat/dsh-tools'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@mutantcat/dsh-session'
...
import { PtcRuntime, type PtcRunRequest, type PtcRunResult } from '@mutantcat/dsh-ptc-runtime'
...
describe('/plan', () => {
  it('registers only when a commands service is composed and optionally submits the next-step message', async () => {
    ...
    expect(ctx.commands.list(plainAgent)).toEqual([
      { definitionId: '@mutantcat/dsh-plan-mode', name: 'plan', description: 'Enter or leave plan mode', input: { hint: '[off|message]', attachments: true } },
    ])
