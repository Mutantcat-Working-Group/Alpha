import { freezeMessage, MessageId } from '@mutantcat/dsh-llm'
...
import { SessionFormatUnsupportedError } from '@mutantcat/dsh-session-persistence'
import type { SessionHandle } from '@mutantcat/dsh-session-persistence'
...
describe('V2 system prompts through current Session and JSONL persistence', () => {
  ...
  it('remaps local references and keeps the inherited cut stable through publication and a current Session append', async () => {
    ...
    try {
      ...
      session.append('system/message', {
        turn: 2, step: 1,
        message: freezeMessage({ role: 'system', id: MessageId('resumed-system'), content: [{ type: 'text', text: 'resumed prompt' }], source: { kind: 'plugin', plugin: '@mutantcat/dsh-system-prompt' } }),
  ...
  it('persists native V3 system appends after the protected head without converting them to user messages', async () => {
    ...
    try {
      ...
      session.append('system/message', {
        turn: 1, step: 1,
        message: freezeMessage({ role: 'system', id: MessageId('head'), content: [{ type: 'text', text: 'head prompt' }], source: { kind: 'plugin', plugin: '@mutantcat/dsh-system-prompt' } }),
      }, { surfaceOp: 'append' })
