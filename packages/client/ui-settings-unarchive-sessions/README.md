...
# @mutantcat/dsh-client-ui-settings-unarchive-sessions
...
Open Settings and select **Archived sessions** to see the sessions currently hidden from every grouping surface. Mount `@mutantcat/dsh-client-ui-settings-unarchive-sessions` in a Web composition that already provides the settings shell, the Workspace service, and the Session list; the page registers its own navigation entry and needs no configuration.
