...
describe('published npm dependency catalog', () => {
  ...
  it('isolates scoped registries, resolver settings, and caches from user, global, and environment configuration', async () => {
    ...
    writeFileSync(userConfig, '@mutantcat:registry=https://user-override.invalid/\nstrict-peer-deps=true\n')
    ...
    expect(JSON.parse(before.output)).toMatchObject({
      '@mutantcat:registry': 'https://user-override.invalid/', 'install-strategy': 'nested',
    })
    ...
    expect(settings).toMatchObject({
      registry: 'https://registry.npmjs.org/', '@mutantcat:registry': 'https://registry.npmjs.org/',
