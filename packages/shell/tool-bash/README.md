...
```yaml
- name: '@mutantcat/dsh-bash-local'
- name: '@mutantcat/dsh-shell-env'
- name: '@mutantcat/dsh-tool-bash'

# Optional: background jobs
- name: '@mutantcat/dsh-jobs-local'
- name: '@mutantcat/dsh-tool-jobs'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-bash) is the exhaustive source for every accepted field and its JSDoc; the generated [tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-bash) carries the full argument schema.
...
A composition with no executor provider never activates the tool. Background calls without the job runtime fail with `background jobs unavailable: load @mutantcat/dsh-jobs and @mutantcat/dsh-tool-jobs`, and `sandbox_permissions` without a sandboxing executor fails with `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`. `enableRunInBackground: false` removes the parameter and rejects a forced background call at execution time.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-bash) — the exact `bash` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-bash) — every accepted config field and its source declaration.
...
The model sees the generated [`bash` schema](../../../docs/tool-catalog.md#mutantcatdsh-tool-bash). `run_in_background` appears only when this producer enables it; `sandbox_permissions` and `justification` appear only when the mounted executor advertises sandboxing. Agent-scoped tool restrictions can remove the definition for that agent.
