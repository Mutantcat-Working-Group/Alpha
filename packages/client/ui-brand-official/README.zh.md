---
description: "面向侧栏与会话首屏的官方 Alpha 品牌填充；供选择或替换品牌呈现的用户与维护者阅读。"
...
在采用 Alpha 自有品牌的部署中，将本插件挂载到浏览器插件名单。标志填充在所有构建 profile 下注册；`official` profile 额外以 Alpha 名称取代本地构建标签。
...
自有身份的部署不组合本包，而是组合另一个占据侧栏与首屏 slot 的包。占据 slot 是唯一的组合路径；这里不存在任何品牌配置面。
...
三个填充作为一组声明感知的注册安装：嵌套的 `ctx.slots.inject()` 调用等待 slot 声明，因此无论本行在声明者之前还是之后激活，这组注册都能工作；声明消失时所有填充一并撤回，HMR 期间也不会留下残缺的品牌混合。两个标志填充在所有 profile 下注册；名称填充仅在 `DSH_CLIENT_BUILD_PROFILE` 为 `official` 时注册。浏览器半部是 [`src/client/index.ts`](src/client/index.ts)；node 半部是一个空 Loader 座位。浏览器标题是构建环境的事（`DSH_CLIENT_TITLE`），不在 slot 系统之内。
