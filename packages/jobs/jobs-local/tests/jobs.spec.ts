...
import { bindScopeParent, createScope, scopeOf } from '@mutantcat/dsh-scope'
import type { ScopeKey } from '@mutantcat/dsh-scope'
import { JobId } from '@mutantcat/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@mutantcat/dsh-jobs'
import LocalJobRegistry, { type Config as JobsConfig } from '@mutantcat/dsh-jobs-local'
import { unsupportedInbox } from '@mutantcat/dsh-agent-loop-testkit'

declare module '@mutantcat/dsh-jobs' {
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @mutantcat/dsh-tool-jobs in its composition)')
  })
