...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-system-prompt) is the exhaustive source for every accepted field. A `toolOrder` list without exactly one rest entry or with duplicates fails at load; a listed name with no registered tool rejects every `assemble()`.
