...
function packageSetProject(): {
  ...
  const dsh = record('@mutantcat/dsh', 'dsh.tgz', dshBody)
  const base = record('@mutantcat/dsh-base', 'dsh-base.tgz', baseBody)
  const host = record('@mutantcat/dsh-desktop-host', 'dsh-desktop-host.tgz', hostBody)
...
describe('desktop core package set', () => {
  it('pins the direct dsh dependency and every internal package to local tarballs', () => {
    ...
    expect(desktopCorePackageOverrides(packageSet)).toEqual({
      '@mutantcat/dsh': 'file:./desktop-packages/dsh.tgz',
      '@mutantcat/dsh-base': 'file:./desktop-packages/dsh-base.tgz',
      '@mutantcat/dsh-desktop-host': 'file:./desktop-packages/dsh-desktop-host.tgz',
    })
  })
  ...
  it('rejects registry resolutions for names supplied by the local package set', () => {
    const dsh = record('@mutantcat/dsh', 'dsh.tgz', Buffer.from('dsh'))
    const host = record('@mutantcat/dsh-desktop-host', 'host.tgz', Buffer.from('host'))
