...
```yaml
- name: '@mutantcat/dsh-pwsh-local'
- name: '@mutantcat/dsh-shell-env'
- name: '@mutantcat/dsh-tool-pwsh'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-pwsh) is the exhaustive source for every accepted field and its JSDoc; the generated [tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-pwsh) carries the full argument schema.
...
A composition with no PowerShell executor never activates the tool, and the injected services (`tools`, `shell`, `systemPrompt`, `shellEnv`) must all exist. Background calls without the job runtime fail with `background jobs unavailable: load @mutantcat/dsh-jobs and @mutantcat/dsh-tool-jobs`, and `sandbox_permissions` without a sandboxing executor fails with `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#mutantcatdsh-tool-pwsh) — the exact `pwsh` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#mutantcatdsh-tool-pwsh) — every accepted config field and its source declaration.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#mutantcatdsh-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@mutantcat/dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls.
