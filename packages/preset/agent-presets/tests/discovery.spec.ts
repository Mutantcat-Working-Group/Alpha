...
import { COMPOSITION_FILE, discoverPresets, scanRoot } from '@mutantcat/dsh-agent-presets'
...
describe('rows naming a plugin that cannot be resolved', () => {
  ...
  it('reports a package the harness cannot resolve, with the row and the name', async () => {
    ...
    // compose a session.
    expect(await scanned('- id: stale\n  name: \'@mutantcat/dsh-no-such-package\'\n'))
      .toBe('row "stale" names a plugin that cannot be resolved: @mutantcat/dsh-no-such-package')
  })
